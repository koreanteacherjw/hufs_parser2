...
class UTagger(object):
    ...
    def Load_global(dll_path, hlxcfg_path):
        ...
        lib.cma_line_depen_json.restype = c_wchar_p #의존 규칙
        lib.cma_ner_js.restype = c_wchar_p #개체명
        lib.cma_erc.restype = c_wchar_p #띄붙 교정
        lib.cma_sk.restype = c_wchar_p #북->남 nk

        lib.cma_tag_line_json2.restype = c_wchar_p #형태소 분석 결과 json으로. 후보 정보나 기타 정보 없이 깔끔.
    ...
    def ner(self, s):
        rt = UTagger.lib.cma_ner_js( self.th_num, u8(s) )
        return rt

    def erc(self, s):
        rt = UTagger.lib.cma_erc( self.th_num, u8(s) )
        return rt

    def sk(self, s):
        rt = UTagger.lib.cma_sk( self.th_num, u8(s) )
        return rt

    def tag_json(self, sent_surf):
        rt = UTagger.lib.cma_tag_line_json2( self.th_num, u8(sent_surf), 0 )
        return rt

    def reload_custom(self):
        #rt = bytes[10000]
        #bug = c_char_p(10000)
        bug = create_string_buffer(10000)
        UTagger.lib.cmaReloadCustom( self.th_num, bug)
        rt = bug.value.decode('cp949')
        return rt
